...
**M16 → M12 → M13.** M15 has landed, and M14, M9 and M10 before it, with M11
before them.

- M16 follows M15 and not the other way round, because its best vehicle is a
  mission and missions did not exist until M15. Both halves of that dependency
  are now settled: missions run, and `best_streak_bpm` was captured during M15
  rather than waiting here, because tempo at the moment a streak was clinched
  can only ever be recorded forwards.
